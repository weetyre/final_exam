...
from .models import MyUser,User_realation
import sqlite3
...
def gadd(request):
    conn = sqlite3.connect('db.sqlite3')
    cursor = conn.cursor()
    value = cursor.execute('select num_of_group from wechat_group where gname = ?',('request',))
    v = value + 1
    cursor.execute('update wechat_group set num_of_group = ? where gname = ?',('v','request',))
    conn.commit()
    conn.close()
